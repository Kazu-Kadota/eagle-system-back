...
import { mockTechmizeNewV1AnswerAnalysisPersonCNHGetResponse } from 'src/mock/techmize/new-v1/answer-analysis/person/cnh/get-response'
...
import { TechmizeNewV1CNHResponseData } from 'src/models/techmize/new-v1/cnh/response'
...
export type TechmizeNewV1AnswerAnalysisCNHBodyValue = PersonSnsMountMessageReturn & {
  retry?: boolean
}

export type TechmizeNewV1AnswerAnalysisCNHBody = {
  person: {
    [PersonAnalysisTypeEnum.CNH_BASIC]: TechmizeNewV1AnswerAnalysisCNHBodyValue
  }
}
...
const techmizeNewV1AnswerAnalysisCNHData: SQSStepFunctionController<TechimzePersonSQSReceivedMessageAttributes> = async (message) => {
  logger.debug({
    message: 'Start on verify if techmize has response for cnh',
  })
  const body = validateBody((message.body as TechmizeNewV1AnswerAnalysisCNHBody).person[PersonAnalysisTypeEnum.CNH_BASIC])
  ...
  const cnh_result: TechmizeNewV1GetResponseResponse = get_response_techimze
    ? await techmizeNewV1GetResponse({
      protocol: body.protocol,
    })
    : mockTechmizeNewV1AnswerAnalysisPersonCNHGetResponse

  if (cnh_result.code === 0) {
    if (cnh_result.message === techmizeNewV1GetResponseNotFinishedErrorResponseMessage) {
      logger.info({
      ...
      const return_body: TechmizeNewV1AnswerAnalysisCNHBody = {
        person: {
  ...
  const cnh = cnh_result.data as TechmizeNewV1CNHResponseData
...
export default techmizeNewV1AnswerAnalysisCNHData
