...
const techmizeV1AnswerAnalysisPersonCNHV2: SQSController<TechimzePersonSQSReceivedMessageAttributes> = async (message) => {
  ...
  const cnh_v2 = cnh_v2_result.data.cnh_v2

  if (cnh_v2 === techmizeV1SuccessErrorResponse) {
    logger.warn({
      message: 'TECHMIZE: Returned success on request cnh v2 result, but received error instead',
      cnh_v2_result,
    })

    throw new ErrorHandler('TECHMIZE: Returned success on request cnh v2 result, but received error instead', 500)
  }
