...
const techmizeV2AnswerAnalysisProcess: SQSStepFunctionController<TechimzePersonSQSReceivedMessageAttributes> = async (message) => {
  ...
  const process_result: TechmizeV2GetResponseResponse | TechmizeV2GetRequestErrorResponse = get_response_techimze
  ...
  const processos = (process_result as TechmizeV2ConsultarProcessosResponseSuccess).data.processos_judiciais_administrativos[0] ?? (process_result as TechmizeV2ConsultarProcessosResponseSuccess).data.processos
