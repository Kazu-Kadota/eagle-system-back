...
export type FeatureFlagBFFSetValidate<T extends FeatureFlagsWithBody> = Array<{
...
const schema = Joi.array().items(featureFlagSchema).required()

const validateBody = <T extends FeatureFlagsEnum>(
  data: Partial<FeatureFlagBFFSetValidate<T extends FeatureFlagsWithBody ? T : never>>,
): FeatureFlagBFFSetValidate<T extends FeatureFlagsWithBody ? T : never> => {
  ...
  if (error) {
    logger.error('Error on validate "set feature flag bff" body')
