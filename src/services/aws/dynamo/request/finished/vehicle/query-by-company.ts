...
export interface QueryFinishedRequestVehicleByCompany {
  start_date: string
  final_date: string
  company_name: string
}
...
export interface QueryFinishedRequestVehicleByCompanyResponse {
  result: VehicleRequest[]
  last_evaluated_key?: string
  count: number
}

const queryFinishedRequestVehicleByCompany = async (
  data: QueryFinishedRequestVehicleByCompany,
  ...
  logger.debug({
    message: 'Querying finished request vehicle by company name with date',
    start_date,
    final_date,
    company_name,
  })
...
export default queryFinishedRequestVehicleByCompany
