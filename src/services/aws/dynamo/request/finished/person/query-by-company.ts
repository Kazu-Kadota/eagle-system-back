...
export interface QueryFinishedRequestPersonByCompany {
  start_date: string
  final_date: string
  company_name: string
}
...
export interface QueryFinishedRequestPersonByCompanyResponse {
  result: PersonRequest[]
  last_evaluated_key?: string
  count: number
}

const queryFinishedRequestPersonByCompany = async (
  data: QueryFinishedRequestPersonByCompany,
  ...
  logger.debug({
    message: 'Querying finished requested person by company name with date',
    start_date,
    final_date,
    company_name,
  })
...
export default queryFinishedRequestPersonByCompany
